...
function bin() {
  ...
  function histogram(data) {
    ...
    if (!Array.isArray(tz)) {
      ...
      tz = ticks(x06, x12, tn);
      if (tz[0] <= x06)
        step = tickIncrement2(x06, x12, tn);
      if (tz[tz.length - 1] >= x12) {
        if (max6 >= x12 && domain === extent) {
          const step2 = tickIncrement2(x06, x12, tn);
          if (isFinite(step2)) {
            if (step2 > 0) {
    ...
    if (isFinite(step)) {
      ...
      for (i = 0; i < n; ++i) {
        if ((x4 = values[i]) != null && x06 <= x4 && x4 <= x12) {
          bins[bisect_default2(tz, x4, 0, m3)].push(data[i]);
        }
      }
    }
    return bins;
...
function quantileSorted2(values, p, valueof = number) {
  if (!(n = values.length) || isNaN(p = +p))
    return;
  if (p <= 0 || n < 2)
...
function nice2(start2, stop, count4) {
  let prestep;
  while (true) {
    const step = tickIncrement3(start2, stop, count4);
    if (step === prestep || step === 0 || !isFinite(step)) {
      return [start2, stop];
    } else if (step > 0) {
      start2 = Math.floor(start2 / step) * step;
...
export {
  ...
  quantileIndex,
  quantileSorted2 as quantileSorted,
  ...
  thresholdSturges,
  tickFormat,
  tickIncrement2 as tickIncrement,
  tickStep2 as tickStep,
  ticks,
  timeDay,
  timeDays,
  timeFormat,
